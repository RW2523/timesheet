...
import { getBatch, listValidation } from '@/lib/api'
import BatchSummaryCards from '@/components/BatchSummaryCards'
import { FileText, AlertTriangle, CheckCircle2, DollarSign, Download } from 'lucide-react'
...
export default function BatchDetailPage({ params }: Props) {
  ...
  const { data: batch, isLoading } = useQuery({
    queryKey: ['batch', id],
    queryFn: () => getBatch(id),
    refetchInterval: (data) => (data?.status === 'PROCESSING' ? 3000 : 30000),
  })
